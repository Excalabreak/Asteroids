...
public class ScreenWrap : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (screenPos.x <= 0 && objectRigidbody.velocity.x < 0)
        {
            transform.position = new Vector3(topRight.x, transform.position.y, 0f);
        }
        else if (screenPos.x >= Screen.width && objectRigidbody.velocity.x > 0)
        {
            transform.position = new Vector3(bottomLeft.x, transform.position.y, 0f);
        }
        if (screenPos.y >= Screen.height && objectRigidbody.velocity.y > 0)
        {
            transform.position = new Vector3(transform.position.x, bottomLeft.y, 0f);
        }
        else if (screenPos.y <= 0 && objectRigidbody.velocity.y < 0)
        {
